...
export default function DashboardGrid() {
  ...
  const createHabit = async () => {
    ...
    try {
      await apiFetch('/habits', {
        ...
        body: JSON.stringify({ 
          title: newHabit, 
          category: newHabit.includes('Water') ? 'health' : 
                   newHabit.includes('Code') ? 'work' : 'general' 
        })
      })
      setNewHabit('')
      loadAllData()
  ...
  const logHabit = async (habitId) => {
    try {
      ...
      loadAllData() // Refresh score + analytics
    } catch (error) {
    ...
    <div className="min-h-screen bg-gradient-to-br from-emerald-50 to-blue-50 p-8">
      <div className="max-w-6xl mx-auto space-y-8">
        ...
        <div className="grid grid-cols-1 md:grid-cols-4 gap-6">
          ...
          <div className="bg-white/80 p-8 rounded-2xl shadow-xl text-center">
            <div className="text-4xl font-black text-orange-600 capitalize">
              {analytics.bestCategory || 'none'}
            </div>
        ...
        <div className="bg-white/80 backdrop-blur-xl rounded-3xl p-8 shadow-2xl">
          ...
          <div className="flex gap-4 max-w-md">
            <input
              ...
              placeholder="e.g. Drink 2L Water, Code 1hr"
            ...
            <button 
              onClick={createHabit}
              className="px-10 py-4 bg-emerald-500 text-white rounded-2xl font-bold text-xl hover:bg-emerald-600 shadow-lg whitespace-nowrap"
            >
              + Add Habit
            </button>
          </div>
        ...
        <div className="bg-white/80 backdrop-blur-xl rounded-3xl p-8 shadow-2xl">
          <div className="flex justify-between items-center mb-8">
            <h2 className="text-3xl font-bold text-gray-800">Your Habits ({habits.length})</h2>
            <button 
              onClick={() => loadAllData()}
              className="px-8 py-3 bg-blue-500 text-white rounded-2xl font-semibold hover:bg-blue-600"
            >
              🔄 Refresh
            </button>
          </div>
          ...
          {habits.length === 0 ? (
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
              {habits.map(habit => (
                <div key={habit.id} className="p-8 bg-gradient-to-br from-emerald-50 to-blue-50 rounded-3xl shadow-xl hover:shadow-2xl transition-all">
                  ...
                  <button 
                    ...
                    className="w-full py-4 bg-emerald-500 text-white rounded-2xl font-bold text-lg hover:bg-emerald-600 shadow-lg transition-all"
                  >
                    ✅ Log Today
                  </button>
                </div>
